...
class DetectFace(object):
    ...
    def  __init__(self, pnet=None, rnet=None, onet=None, min_face=40, device="cuda"):
    ...
    def detect_pnet(self, im):
        ...
        all_boxes = list()
        while current_height >= net_size and current_width >= net_size:
        #while current_height >= 40 and current_width >= 40:
            feed_imgs = []
            #- transform unit8 -> range [0, 1]
            ...
            #- cls, bbox
            net_stt = time.time()
            cls_map, reg = self.pnet_detector(feed_imgs)   # CORE， Don't look landmark

            net_end = time.time() - net_stt
            cls_map_np = cls_map.detach().cpu().numpy()
            reg_np = reg.detach().cpu().numpy()
            boxes = self.generate_bbox(cls_map_np, reg_np, current_scale, self.thresh[0])
            ...
            #boxes = boxes.detach().cpu().numpy() # tensor to np
    ...
    def detect_rnet(self, im, dets):
        ...
        #print("Rnet:", feed_imgs.shape)
        cls_map, reg = self.rnet_detector(feed_imgs) # CORE

        cls_map_np = cls_map.detach().cpu().numpy().astype(np.float32)
        reg_np     = reg.detach().cpu().numpy().astype(np.float32)
    ...
    def detect_onet(self, im, dets):
        ...
        cls_map, reg = self.onet_detector(feed_imgs)  # look all
