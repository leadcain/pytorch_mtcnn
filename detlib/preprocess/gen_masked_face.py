...
def gen_data(args, argument):
    ...
    img_save_dir = os.path.join(args.save_dir, '%s/masked_face' % folder_name)
    for folder in [txt_save_dir, img_save_dir]:
        if not os.path.exists(folder):
            os.makedirs(folder)

    save_txt_name = 'masked_face_%d.txt' % args.img_size
    ...
    for idx, annotation in enumerate(annotations):
        ...
        if (idx + 1) % 100 == 0:
            print("%d images done, masked face images: %d" % (idx+1, l_idx))
        ...
        if argument:
            ...
            for i in range(args.num_rands):
                bbox_size = np.random.randint(int(min(gt_w, gt_h) * 0.90), np.ceil(1.10 * max(gt_w, gt_h)))
                ...
                if iou > args.threshold:
                    ...
                    #####################################
                    if random.choice([0,1]) > 0:
                        rotated_im = rotate(img, bbox, random.randint(5, 20)) 
                    ...
                    if random.choice([0,1]) > 0:
                        inv_rotated_im = rotate(img, bbox, -random.randint(5, 20))
...
def gen_config():
    parser = argparse.ArgumentParser(description=' Generate masked face file')
    parser.add_argument('--root_dir',  type=str,  default='/datasets/WIDER_FACE')
    parser.add_argument('--anno_file', type=str,  default='/datasets/WIDER_FACE/masked_face/masked_face.txt')
    parser.add_argument('--data_dir',  type=str,  default='/datasets/WIDER_FACE/masked_face')
