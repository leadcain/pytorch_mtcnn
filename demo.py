...
onet_file='model/checkout/onet/onet_epoch_20.pt'

#pnet_file=None 
#rnet_file=None
#onet_file=None

def demo_cam(w=320, h=240, min_face=24, threshold=0.9, device='cpu', interp=cv2.INTER_LINEAR):
    
    mtcnn_detector = DetectFace(
                                pnet=pnet_file,
                                rnet=rnet_file,
                                onet=onet_file,
                                min_face=min_face, device=device)
    ...
    while(cap.isOpened()):
        ...
        dlib.easy_vis(disp_img, bboxs, None, scale=(w_scale, h_scale), threshold=threshold, save_path=None)
...
def demo_video(filename, speed=1, w=320, h=240, min_face=24, device='cpu', interp=cv2.INTER_LINEAR):
    ...
    sync_time = frame_syncronizer(fps=fps)
    print("sync time is {:}".format(sync_time))
    ...
    while(cap.isOpened()):
        ...
        if cv2.waitKey(sync_time) & 0xFF == ord('q'):
            break
...
def frame_syncronizer(fps=1000):
    return int(1000. / fps)
...
if __name__ == '__main__':
    ...
    demo_cam(240, 320, min_face=48, threshold=0.9, device='cpu')
    #demo_cam(200, 270, min_face=24, device='cpu')
    #demo_dir()
    #demo_video("videos/test.mp4", speed=32, w=240, h=320, min_face=24)
